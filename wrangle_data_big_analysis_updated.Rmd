...
eeg_data.1 <-
  ...
  dplyr::select(
    ID = Subject,
    lnTheta_Bingo_F3_30mo:lnAlpha_Bingo_T8_30mo,
    lnTheta_Bingo_F3_42mo:lnAlpha_Bingo_T8_42mo,
    lnTheta_Open_F3_8:lnAlpha_Open_T8_8,
    EEG_lnF3Theta_EO_12:EEG_lnT4Theta_EO_12,
    EEG_lnF3Alpha_EO_12:EEG_lnT4Alpha_EO_12
  ) %>%
  mutate(
    theta_30 = pmap_dbl(
  ...
  left_join(
    read_tsv(EEG_16yr_data_file) %>% 
      dplyr::select(
        ID = subnum,
        F3_EO_16Y_abs_theta:O2_EO_16Y_abs_theta,
        F3_EO_16Y_abs_alpha:O2_EO_16Y_abs_alpha
      )
  ) %>% 
  mutate(
    theta_16 = pmap_dbl(
      select(., F3_EO_16Y_abs_theta:O2_EO_16Y_abs_theta),
      function(...) mean((c(...)))
    ),
    alpha_16 = pmap_dbl(
      select(., F3_EO_16Y_abs_alpha:O2_EO_16Y_abs_alpha),
      function(...) mean((c(...)))
    )
  )
...
eeg_data.2 <- 
  eeg_data.1 %>% 
  select(ID, theta_30:alpha_12, theta_16, alpha_16) %>% 
...
eeg_16 %>% 
  filter(str_detect(eeg_key, "theta") == TRUE) %>% 
  ggplot(aes(value)) +
...
eeg_data.2 %>% 
  ...
  theme_beip +
  labs(
    x = "16-year wave absolute power\n(mean across all available electrode sites)"
  )
```
...
eeg_data.2 %>% 
  ...
  summarise_at(
    vars(value),
    funs(mean, median, sd, min, max)
  ) %>%
  ggplot(aes(measure, mean)) +
  geom_col() +
  facet_grid(.~wave)

ggsave("~/Desktop/EEG_absolute_wave.png")
```
...
eeg_data_wf <-
  ...
  select(
    ID,
    lnTheta_Bingo_F3_30mo,
    lnTheta_Bingo_F3_42mo
  )
...
# Export data
```{r}
write_csv(d, paste0(home, "/data/BEIP_big_analysis_tidied_data_20211022.csv"))
```
