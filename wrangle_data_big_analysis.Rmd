...
EEG_data_file <- paste0(home, "data/EYESOPEN_EYESCLOSED_EEG_ALL_USE_forKate.sav")
```
...
eeg_data <-
  ...
  dplyr::select(Subject, contains("alpha")) %>% 
  rename(
    ID = Subject
  ) %>% 
...
For info on separating b/w and within-subject components of stability: https://solomonkurz.netlify.app/post/2019-10-31-time-varying-covariates-in-longitudinal-multilevel-models-contain-state-and-trait-level-information-this-includes-binary-variables-too/
```{r}
d <-
  d0_tidy %>% 
...
# Export data
```{r}
write_csv(d, paste0(home, "/data/BEIP_big_analysis_tidied_data_20210723.csv"))
```
